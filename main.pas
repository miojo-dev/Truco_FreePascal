...
procedure ShowPlayerHand(H: TPlayer);
var i : integer;
begin
    writeln;
    ...
    for i := 1 to 3 do
        begin
            if H.hand[i].weight = gameManager.CurrentManilha then
                writeln(' - [', i, '] ', H.hand[i].weight,
                    ' of ', SuitStr(H.hand[i].suit),'  **MANILHA**')
                
            else
                writeln(' - [', i, '] ', H.hand[i].weight,
                    ' of ', SuitStr(H.hand[i].suit));
     end;
end;
...
begin
    ...
    until IdxP >= 1;
        ...
        {Computer chooses}
        IdxC := ChoosePCcard(round).weight;
